...
provides an OAI repository at \url{https://epub.wu.ac.at/cgi/oai2}.
...
%% (\url{https://dublincore.org/documents/dcmi-terms/#terms-language}) says
%% ``Recommended best practice is to use a controlled vocabulary such as
%% RFC 4646'', which (see \url{https://www.rfc-editor.org/rfc/rfc4646})
